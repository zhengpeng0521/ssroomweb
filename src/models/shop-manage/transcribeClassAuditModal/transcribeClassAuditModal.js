...
import {
  ...
  queryBenefit,
  auditGoods,
} from '../../../services/shop-manage/transcribeClassAuditService';

import {
  getPlatGoods,
  queryPlatGoodsAdditionalInfo,
} from '../../../services/member-card-manage/zygHqspreadGoodsAuditService';
import { message, } from 'antd';
import moment from 'moment';

export default {
  namespace: 'transcribeClassAuditModal',

  state: {
    TopDeductAmountDetail : {}, //佣金详情弹出框里的数据
    isTopDeductAmount : false,  //是否显示佣金详情
    /*表格项*/
    loading: false,
    appointOther: [], //预约其他想选
  ...
  subscriptions: {
    setup({ dispatch, history, }) {
      history.listen(({ pathname, query, }) => {
        if (pathname == '/zyg_transcribe_class_audit') {
          dispatch({
            type : 'updateState',
            payload : {
              showDropdown : false
  ...
  effects: {
    //获取待审核商品列表
    *queryAudit({ payload, }, { call, put, }) {
      yield put({ type: 'showLoading', });
      const { pageIndex, pageSize, } = payload;
      const { ret, } = yield call(queryAudit, {
        pageIndex,
        pageSize,
      });
    ...
    *queryBenefit({ payload, }, { select, call, put, }) {
      const state = yield select(state => state.transcribeClassAuditModal);
      ...
      if (ret && ret.errorCode == '9000') {
        yield put({
          ...
          payload: {
            ...
            TopDeductAmountDetail : ret
          },
        });
      } else {
        message.error((ret && ret.errorMessage) || '查看佣金详情失败');
      }
    },

    /* 分销商品审核 */
    *auditGoods({ payload, }, { select, call, put, }) {
      const state = yield select(state => state.transcribeClassAuditModal);
      const { ret, } = yield call(auditGoods, payload);
      if (ret && ret.errorCode == '9000') {
        message.success((ret && ret.errorMessage) || '商品审核成功');
        yield put({
          type: 'queryAudit',
          payload: {
            pageIndex: 0,
        ...
        message.error((ret && ret.errorMessage) || '分销商品审核失败');
      }
    },
