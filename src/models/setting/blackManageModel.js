...
export default {
  ...
  state: {
    ...
    searchContent: {},
    //添加黑名单相关变量
    addWhiteListModalTitle: '黑名单账号设置',//加黑名单Modal标题
    addWhiteListModalVisible: false,//添加黑名单Modal是否显示
    //设置黑名单相关变量
    whiteDefaultValue: [],//设置黑名单页面中已经是黑名单的要勾选
    mobile: '',
    setWhiteListModalTitle: '黑名单账号设置',
    ...
    cardType: [],
    //查看黑名单相关变量
    lookWhiteListDrawerTitle: '黑名单信息',
  ...
  effects: {
    ...
    *queryWhiteList({ payload, }, { select, call, put, }) {
      ...
      if (ret && ret.errorCode == '9000') {
        ...
        message.error((ret && ret.errorMessage) || '黑名单列表加载失败');
      }
      yield put({ type: 'updateState', payload: { loading: false, }, });
    },
    //根据手机号查询黑名单
    ...
    *update({ payload, }, { select, call, put, }) {
      const state = yield select(state => state.BlackManageModel);
