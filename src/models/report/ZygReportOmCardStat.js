...
import { queryAsyncList, downloadTemplate } from '../../services/report/ZygReportOmCardStat'
	...
	subscriptions: {
		setup({ dispatch, history, }) {
			history.listen(({ pathname, query, }) => {
				if (pathname == '/zyg_report_om_card_stat') {
          dispatch({
            ...
            payload: {
					...
					dispatch({
						...
						payload: {
							...
							searchContent : {
                exportFlag : false,
                orderTimeStart : moment().format('YYYY-MM-DD'),
                orderTimeEnd : moment().format('YYYY-MM-DD'),
							}
						},
					});
					// setTimeout(() => {
