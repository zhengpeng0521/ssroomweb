...
import { queryAsyncList, downloadTemplate } from '../../services/report/ZygReportOmDetail'
	...
	subscriptions: {
		setup({ dispatch, history, }) {
			history.listen(({ pathname, query, }) => {
				if (pathname == '/zyg_report_om_detail') {
					dispatch({
						type: 'queryAsyncList',
						payload: {
							pageIndex: 0,
              ...
              searchContent : {
                exportFlag : false,
                visitTimeStart :moment().format('YYYY-MM-DD'),
                visitTimeEnd :moment().format('YYYY-MM-DD'),
              }
						},
					});
					// setTimeout(() => {
