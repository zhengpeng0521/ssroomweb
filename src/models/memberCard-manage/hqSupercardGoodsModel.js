...
export default {
  ...
  state: {
    stockShowVipCardVisible : true,  //库存弹出框顶部是否显示选择会员卡
  ...
  effects: {
    ...
    *queryTenantList({ payload, }, { call, put, }) {
    ...
    //商品预约排期
    *setPlatGoodsAppoint({ payload, }, { select, call, put, }) {
      yield put({ type: 'showLoading', });
      // message.loading({
      //   content : '提交中'
      // });
      // return false;
      const { ret, } = yield call(setPlatGoodsAppoint, payload);
      if (ret && ret.errorCode == '9000') {
    ...
    *queryAuditGoods({ payload, }, { call, put, }) {
      yield put({ type: 'showLoading', });
      const { pageIndex, pageSize, searchContent, useStatus} = payload;
    ...
    *tableColumnSave({ payload, }, { select, call, put, }) {
      ...
      const state = yield select(state => {
        return state.hqSupercardGoodsModel;
      });
