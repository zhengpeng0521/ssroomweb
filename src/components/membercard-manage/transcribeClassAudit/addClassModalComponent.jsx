...
import styles from './addClassModalComponent.less';
...
function AddTicketComponent({
  ...
  function queryTenantShop(tenantId) {
    dispatch({
      type : 'hqSupercardGoodsModel/queryTenantShop',
      payload : {
        tenantId
      }
    });
  ...
  const confirmCreateAction = () => {
    validateFieldsAndScroll((err, values) => {
      ...
      if(values.fragNum === null || values.fragNum === undefined || values.fragNum === ''){
        ...
        values.fragNum = String(values.fragNum);
      }
      ...
      <Form>
          ...
          getFieldValue('goodsType') == '103' ? (
            ...
            >
              {getFieldDecorator('fragNum', {
                ...
                <InputNumber placeholder={'预约课程返利惠豆数量'} style={{width : 160}} />
                // <div>
                //   <InputNumber />
        ...
        >
          ...
          <span style={{ display: '-webkit-box', color: '#8c8c8c', }}>
            请输入售卖价格(大于0的数字，最多到小数点后2位，不大于原价){' '}
          </span>
        </FormItem>
        <FormItem {...formItemLayout}
          label="结算价"
        >
          ...
          <span style={{ display: '-webkit-box', color: '#8c8c8c', }}>
            请输入结算价(大于0的数字，最多到小数点后2位，不大于原价和售卖价){' '}
          </span>
        </FormItem>
