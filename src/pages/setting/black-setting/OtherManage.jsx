const namespace = 'BlackManageModel';
import React from 'react';
import { connect, } from 'dva';
import { Card, Button, message, Modal } from 'antd';
import AddWhiteListComponent from '../../../components/setting/black-manage/addWhiteListComponent'
import SetWhiteListComponent from '../../../components/setting/other-manage/setWhiteListComponent'
// import SetWhiteListComponent from '../../../components/setting/black-manage/setWhiteListComponent'
import LookWhiteListComponent from '../../../components/setting/black-manage/lookWhiteListComponent'

function BlackManage({ dispatch, BlackManageModel }) {
  ...
  function pageOnChange(pageIndex, pageSize) {
    dispatch({
      type: 'BlackManageModel/pageChange',
      payload: {
        pageIndex,
        pageSize,
      },
  ...
  function showAddWhiteListModalFn() {
    dispatch({
      type: 'BlackManageModel/updateState',
      payload: {
  ...
  function setSearchPhoneNumFn(val) {
    dispatch({
      type: 'BlackManageModel/getByMobile',
      payload: {
        mobile: val
      }
    })
  ...
  function showLookWhiteListDrawerFn() {
    dispatch({
      type: 'BlackManageModel/queryWhiteList',
      payload: {
        pageIndex: 0,
        pageSize: 20,
      },
  ...
  function removeWhiteListFn(id) {
    dispatch({
      type: 'BlackManageModel/deleteWhiteList',
      payload: {
        id
      }
    })
    ...
    },
    {
      ...
      render: (text, record) => (
        <div>
          ...
          <Button disabled={record.status != 1} style={{ marginLeft: '10px',background:'#FF8989',borderColor:'#FF8989',color:'#fff' }} onClick={removeWhiteListFn.bind(this, record.id)}>移除黑名单</Button>
        </div>
      ),
    }
  ];
  ...
  function saveColumns(val) {
    dispatch({
      type: 'BlackManageModel/tableColumnSave',
      payload: {},
    });
  }
  ...
  const TicketComponentProps = {
    search: {
      ...
      fields: [
        { key: 'id', type: 'input', placeholder: '黑名单编号', },
        ...
        },
        {
          ...
          width: '290px',
          format: 'YYYY-MM-DD',
          startPlaceholder: '黑名单创建开始时间',
          endPlaceholder: '黑名单创建结束时间',
        },
        {
          key: 'expireTime',
          type: 'rangePicker',
          ...
          width: '290px',
          format: 'YYYY-MM-DD',
          startPlaceholder: '黑名单过期开始时间',
          endPlaceholder: '黑名单过期结束时间',
        },
      ],
    },
    rightBars: {
...
function mapStateToProps({ BlackManageModel, }) {
    return { BlackManageModel, }
}

export default connect(mapStateToProps)(BlackManage)